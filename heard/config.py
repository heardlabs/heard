...
DEFAULTS: dict[str, Any] = {
    ...
    "openai_api_key": "",
    "elevenlabs_api_key": "",
    # Speechify (Simba 3.2) — the other BYOK voice provider. Sits BELOW
    # elevenlabs_api_key in the daemon's backend ladder, so an existing
    ...
    # Values: "trial" | "pro" | "power" | "expired" | "" (unknown / never signed up)
    "heard_plan": "",
    ...
    "phone_paired": False,
    # --- MCP connector (open source) -------------------------------------
    # Local MCP server that lets cloud agents (Grok Bot first) narrate through
    # Heard and hear the user's replies. See heard/mcp_server.py.
    # Secret path segment for the public URL; generated by `heard install
    # grok-bot`, rotated by `heard mcp key --rotate`. Empty = never installed.
    "mcp_key": "",
    # Loopback port the server binds. The tunnel fronts this.
    "mcp_port": 7391,
    # "auto" (cloudflared if installed, else ngrok, else none) | "cloudflared"
    # | "ngrok" | "none" (you front the port yourself; set mcp_public_url).
    "mcp_tunnel": "auto",
    # ngrok static domain (free accounts get one) — a stable URL so the user
    # never re-pastes into Grok. Ignored for cloudflared.
    "mcp_tunnel_domain": "",
    # When mcp_tunnel is "none": the https origin you expose the port on.
    "mcp_public_url": "",

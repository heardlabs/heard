...
| `heard/adapters/claude_code.py` + `codex.py` | Install / uninstall the hook into `~/.claude/settings.json` and `~/.codex/hooks.json`. Codex also owns `codex_enabled`, the app-level preference that lets Codex Desktop log-tailing run even when CLI hooks are absent. PYTHONHOME-wrapped command for the .app bundle case. JSON read-modify-write is duplicated between the two — factor candidate. |
...
| `heard/ui.py` | rumps menu bar. Persona / Speed / Verbosity submenus, Active agents (multi-agent router state), Options, "Pause Heard" / "Resume Heard" toggle, status header (`On · Persona · Profile`, `Paused` when muted, `● Speaking` when active, `⚠ <kind>` on error). Resume with queued items opens a two-button `prompt_window.ask_choice` panel ("Catch Me Up" / "Start Fresh") and routes the selected text through the existing `resume_intent` socket cmd. "Report a problem…" menu item (the ONLY user-facing feedback surface — see memory `heard-product-surface-ambient-utility`) opens the defect dialog via `prompt_window.ask_defect_report` and routes through the `report_defect` socket cmd; daemon auto-attaches tech_context. Notify-on-ack so the user knows the report filed. |
...
| `heard/prompt_window.py` | Native modal-dialog helpers. PyObjC-only — AppKit imports are lazy inside each function so importing the module doesn't pull AppKit on a CLI path. Surfaces today: `ask_choice(title, message, choices, cancel_choice)` → `ChoiceResult(submitted, choice)` for two- or three-button decisions (used by resume-from-pause), `ask(title, message, ...)` → `PromptResult(submitted, text)` for one-line text input, and `ask_defect_report()` → `DefectResult(submitted, category, note)` for the "Report a problem" dialog (NSAlert + NSStackView with NSPopUpButton + NSTextField). Defect categories live in module-level `_DEFECT_CATEGORIES` — an invariant test (`tests/test_prompt_window_categories.py`) keeps them in sync with `defects.CATEGORIES`. Must be called from the main thread (rumps callbacks already are). |

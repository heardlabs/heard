...
| `heard/multi_agent.py` | Solo / Swarm / Pinned router. Decides per-event: speak / drop / defer-to-digest. Project-keyed channel scheduler batches background-agent activity into narrative summaries, with template fallback. `format_digest`, `drain_session_summary`, `pin`/`unpin`, `list_active`. Per-agent voices in swarm mode: `agent_voices` (manual repo→voice map, always wins) → deterministic auto-pool, non-focus sessions only. The pool follows the ACTIVE TTS backend via `voice_pool_for_backend` — provider voice-ID namespaces don't overlap, and the wrong pool collapses every agent onto one voice silently rather than erroring. `multi_agent_voice_scope` picks what a voice is keyed to: `project` (default — hashed on repo_name, stable across restarts, focus agent exempt) or `window` (round-robin per session, covers the focus agent, resets on restart). |
...
| `heard/tts/elevenlabs.py` + `tts/speechify.py` + `tts/kokoro.py` + `tts/managed.py` + `tts/null.py` | TTS backends. Selector order in `Daemon._make_tts`: signed-in Heard token → `ManagedTTS` (proxies api.heard.dev); else BYOK `elevenlabs_api_key` → `ElevenLabsTTS`; else BYOK `speechify_api_key` → `SpeechifyTTS` (Simba 3.2); else if the Kokoro model is already downloaded → `KokoroTTS`; else `NullTTS`. BYOK keys are gated the same way for every provider — an active paid plan keeps its managed voices unless `byok_enabled`. Kokoro is opt-in only — never auto-downloaded. Speechify diverges from the others in three ways: JSON response with base64 audio, speed via SSML `<prosody rate>` (no speed param), and its own voice-ID namespace (`speechify_voice` / `Persona.speechify_voice`, mirroring the `kokoro_voice` split). |
